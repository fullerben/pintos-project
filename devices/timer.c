...
// Comparator method for adding elements to sleeping_list
static bool timer_compare(struct list_elem *tmp1, struct list_elem *tmp2);
...
static bool timer_compare(struct list_elem *elem1, struct list_elem *elem2) {
  struct thread *tmp1;
  struct thread *tmp2;
  ...
  // Compare the threads' wakeup times
  bool done = tmp1->wakeup_time < tmp2->wakeup_time;
  return done;
}
...
timer_sleep (int64_t ticks) 
{
  ...
  // Assert interrupts are enabled, set current interrupt level and disable interrupts temporarily
  ASSERT (intr_get_level () == INTR_ON);
  enum intr_level old_level;
  old_level = intr_disable(); 

  // Get the current thread and prepare for sleep
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  // Loop through the currently sleeping threads
  ...
  while(!list_empty(&sleeping_list)) {
    ...
    // Otherwise, remove the front thread and unblock it
